...
interface MeetingMentionsEditorProps {
  meetingId: string;
}

export function MeetingMentionsEditor({ meetingId }: MeetingMentionsEditorProps) {
  ...
  useEffect(() => {
    const fetchMentions = async () => {
      try {
        const response = await fetch(`/api/meetings/${meetingId}/mentions`);
        if (response.ok) {
    ...
    <Card className="border-blue-200/50 shadow-sm">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="meeting-mentions" className="text-sm font-medium text-slate-700">
