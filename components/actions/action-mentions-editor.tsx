...
interface ActionMentionsEditorProps {
  actionId: string;
}

export function ActionMentionsEditor({ actionId }: ActionMentionsEditorProps) {
  ...
  useEffect(() => {
    const fetchMentions = async () => {
      try {
        const response = await fetch(`/api/actions/${actionId}/mentions`);
        if (response.ok) {
    ...
    <Card className="border-blue-200/50 shadow-sm">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="action-mentions" className="text-sm font-medium text-slate-700">
